...
// Lirester from Telegram limits overflow (Approve!)
...
// It means that only N per T operations are allowed.
//
// 2. Lirester object has a map from chat id (core/chat.ID)
// to the lirester chat object (lirchat), which contains already sent
// messages counter.
// If this counter decreases every some time (T'), and when this counter reaches
// a certain MAX value (X), the message won't be sent to the linked chat,
